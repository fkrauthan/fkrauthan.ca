...
import type { OpensourceData, OpensourceProject } from "./types";

const dataOpensourcePath = path.join(process.cwd(), "data", "opensource");

async function findAllOpensourceProjects(): Promise<string[]> {
  return (await fs.readdir(dataOpensourcePath)).filter((file) => file.endsWith(".md"));
}

async function getOpenSourceData(file: string): Promise<OpensourceProject> {
  ...
  const [, pos, slug] = /^(\d+)-(.+)\.md$/.exec(file) as RegExpExecArray;
  ...
  return {
    ...
    meta: meta as OpensourceProject["meta"],
    markdown,
  };
}

export async function loadOpensourceData(): Promise<OpensourceData> {
